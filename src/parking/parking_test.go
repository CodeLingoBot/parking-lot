...
func testVechicle(index Index) *vehicle.Vehicle {
...
func addCheck(index Index, pC *Center, t *testing.T) {
	oSlot, err := pC.AddVehicle(testVechicle(index))
	if nil != err {
		t.Error(Nil)
	}
	if nil == oSlot {
...
func TestCenter_Functional(t *testing.T) {
	...
	for index = 0; uint64(index) < uint64(capacity); index++ {
		if pC.slots[index].Vehicle.Number != testVechicle(Index(index)).Number {
			t.Error(NoOrder)
		}
	}
	for index = 0; uint64(index) < uint64(capacity); index++ {
...
func BenchmarkCenter_AddVehicle(b *testing.B) {
	var (
		start           Index    = 10000
		index, capacity Capacity = 0, 1000000
		pC                       = New(start, capacity)
	)
	for ; uint64(index) < uint64(capacity); index++ {
		pC.AddVehicle(testVechicle(Index(index)))
	}
}
...
func BenchmarkCenter_AddAndRemove(b *testing.B) {
	...
	for ; uint64(index) < uint64(capacity); index++ {
		pC.RemoveVehicleByNumber(testVechicle(Index(index)).Number)
	}
}
